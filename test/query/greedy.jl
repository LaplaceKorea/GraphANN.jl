@testset "Testing GreedySearch Methods" begin
    ...
    GraphANN.DiskANN.pushcandidate!(x, Neighbor(1, 0.0))

    # Use `egal` for comparison in case we want to overload `==`
    @test maximum(x) === Neighbor(1, 0.0)
    GraphANN.DiskANN.pushcandidate!(x, Neighbor(2, 10.0))
    @test maximum(x) === Neighbor(2, 10.0)

    GraphANN.DiskANN.pushcandidate!(x, Neighbor(3, -10.0))
    @test maximum(x) === Neighbor(2, 10.0)

    GraphANN.DiskANN.reduce!(x)
    @test maximum(x) === Neighbor(1, 0.0)
end
