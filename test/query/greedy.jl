@testset "Testing GreedySearch Methods" begin
    ...
    x = GraphANN.GreedySearch(2; cost_type = Float64)
    GraphANN.Algorithms.pushcandidate!(x, Neighbor(1, 0.0))

    # Use `egal` for comparison in case we want to overload `==`
    @test maximum(x) === Neighbor(1, 0.0)
    GraphANN.Algorithms.pushcandidate!(x, Neighbor(2, 10.0))
    @test maximum(x) === Neighbor(2, 10.0)

    GraphANN.Algorithms.pushcandidate!(x, Neighbor(3, -10.0))
    @test maximum(x) === Neighbor(2, 10.0)
