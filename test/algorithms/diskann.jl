...
function test_index(
    dataset::Vector{SVector{N,T}}, graph_type = GraphANN.DefaultAdjacencyList{UInt32}
) where {N,T}
    alpha = 1.2
    max_degree = 70
    window_size = 50
...
@testset "Testing DiskANN Based Code" begin
    ...
    @testset "Testing Index" begin
        ...
        dataset = GraphANN.load_vecs(
            SVector{128,Float32}, dataset_path
        )::Vector{SVector{128,Float32}}
        ...
        mktempdir(@__DIR__) do dir
            ...
            superflat = GraphANN.load(
                GraphANN.SuperFlatAdjacencyList{UInt32}, original_save
            )
            ...
            # Test `save_as_superflat`
            tmppath = joinpath(dir, "superflat")
            superflat = GraphANN.save_as_superflat(tmppath, default)
            graphs_equal(superflat, default)
            superflat_loaded = GraphANN.load_bin(
                tmppath, GraphANN.SuperFlatAdjacencyList{UInt32}
            )
            graphs_equal(superflat, superflat_loaded)
            graphs_equal(default, superflat_loaded)
        end
    end
    ...
    @testset "Compare With DiskANN Baseline" begin
        ...
        for (i, comparison) in enumerate(comparisons)
            ...
            ids = GraphANN.search(
                algo, meta, queries; num_neighbors = comparison.num_neighbors
            )
