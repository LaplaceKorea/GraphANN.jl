function _run_tpt_tests(data::AbstractVector{SVector{N,T}}) where {N,T}
    ...
    # Make sure that if when we make a thread local copy that all threads share the same
    # permutation.
    runner_tls = GraphANN.ThreadLocal(runner)
    for _runner in GraphANN.getall(runner_tls)
        @test _runner.permutation === runner.permutation
        @test _runner.partitioner !== runner.partitioner
        @test _runner.scratch !== runner.scratch
    end
    ...
    @inferred GraphANN.Algorithms.getweights!(
        data,
        runner,
        dims,
        1:length(data),
    ...
    weights, meanval = GraphANN.Algorithms.getweights!(
        data,
        runner,
        dims,
        1:length(data),
